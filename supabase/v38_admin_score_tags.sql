...
create unique index if not exists user_tags_user_name_ci_key
  on public.user_tags (user_id, lower(btrim(name)));
create index if not exists user_tags_user_order_idx
  on public.user_tags (user_id, sort_order);

create table if not exists public.user_score_tags (
  user_score_id uuid not null references public.user_scores(id) on delete cascade,
  tag_id uuid not null references public.user_tags(id) on delete cascade,
  created_at timestamptz not null default now(),
  primary key (user_score_id, tag_id)
);

alter table public.user_tags enable row level security;
alter table public.user_score_tags enable row level security;

drop policy if exists user_tags_admin_select on public.user_tags;
create policy user_tags_admin_select on public.user_tags for select to authenticated
  using (user_id = auth.uid() and public.is_admin());

drop policy if exists user_score_tags_admin_select on public.user_score_tags;
create policy user_score_tags_admin_select on public.user_score_tags for select to authenticated
  using (
    public.is_admin()
    and exists (select 1 from public.user_scores s where s.id = user_score_id and s.user_id = auth.uid())
    and exists (select 1 from public.user_tags t where t.id = tag_id and t.user_id = auth.uid())
  );
...
begin
  ...
  -- 「A」と「B」の名前を入れ替える場合も一意制約へ衝突しないよう、
  -- 所有する既存タグを一度だけ内部用の仮名へ退避する。
...
begin
  ...
  if not exists (select 1 from public.user_scores where id = p_score_id and user_id = auth.uid()) then
...
grant select on public.user_tags, public.user_score_tags to authenticated;
